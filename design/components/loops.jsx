// Loops — hero feature. Pattern library with constellation visualization.
      ...
      <div style={{ flex: 1, display: 'flex', minHeight: 0 }}>
        <div style={{ width: 340, flexShrink: 0, borderRight: '1px solid var(--rule)', overflowY: 'auto', background: 'var(--paper)' }}>
          ...
          <div style={{ padding: '0 12px 24px' }}>
            ...
            {loops.filter(l => l.state === 'active').map((l, i) => <LoopItem key={i} loop={l} active={selected === i} onClick={() => setSelected(i)} />)}
            ...
            {loops.filter(l => l.state === 'resolved').map((l, i) => {
              const idx = loops.indexOf(l);
              return <LoopItem key={idx} loop={l} active={selected === idx} onClick={() => setSelected(idx)} />;
            })}
          </div>
        </div>
...
Object.assign(window, { LoopsScreen });
