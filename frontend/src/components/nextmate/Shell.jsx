...
export const Icon = ({ name, size = 14, style }) => {
  const P = {
    ...
    loops: (
      <>
        <path d="M8 2a6 6 0 106 6" />
        <path d="M14 4v4h-4" />
      </>
    ),
    insights: (
      <>
        <path d="M2 13h12M4 10V7M7 10V4M10 10V6M13 10V3" />
      </>
    ),
    weekly: (
      <>
    ...
    book: (
      <>
        <path d="M3 2h7a2 2 0 012 2v10H5a2 2 0 01-2-2V2z" />
        <path d="M3 2v10M6 5h4M6 8h4" />
      </>
    ),
    trash: (
      <>
...
const NavItem = ({ icon, label, k, active, onNav, count }) => (
  <button
    className={"nm-nav-item" + (active === k ? " active" : "")}
    onClick={() => onNav && onNav(k)}
  >
    <span className="nm-nav-ic">
      <Icon name={icon} />
    </span>
    <span>{label}</span>
  ...
  return (
    <aside className="nm-side">
      <div>
        ...
        <div className="nm-side-menu">
          <button
            className="nm-btn accent begin-btn"
            onClick={onNewThread}
            style={{
          ...
          <div className="nm-threads">
            {threads.length === 0 && (
              <div className="nm-meta" style={{ padding: "8px 10px" }}>
                No threads yet.
              </div>
            )}
            {threads.map((t) => {
              const isActive =
                t.thread_id === activeThreadId && active === "chat";
              return (
                <div
                  key={t.thread_id}
...
export const LoopRing = ({
  ...
  const r = size / 2 - 5;
  const c = 2 * Math.PI * r;
  const color = resolved ? "var(--accent-2)" : "var(--accent)";
  return (
    <div
    ...
    >
      <svg width={size} height={size} style={{ transform: "rotate(-90deg)" }}>
        ...
        <circle
          ...
          stroke={color}
          strokeWidth="1"
          strokeDasharray={`${(c - 10) * strength * 0.6} ${c}`}
          opacity="0.35"
        />
      </svg>
      {showLabel && (
        ...
        >
          ...
          <div className="nm-meta" style={{ fontSize: 8.5 }}>
            loop
          </div>
        </div>
      )}
