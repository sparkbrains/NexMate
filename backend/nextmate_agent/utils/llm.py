...
def _get_gemini_chat_model(model_env_var: str = "GENERATION_MODEL") -> tuple[_GeminiClient, str]:
    ...
    notice; override via GENERATION_MODEL or FAST_MODEL if you want a specific pinned
    version instead."""
...
def get_chat_model():
    ...
    Always reads current environment/.env values and builds a fresh client --
    nothing is cached across calls."""
    return _get_gemini_chat_model("GENERATION_MODEL")


def get_fast_chat_model():
    """Return a (client, model_name) tuple for Gemini using FAST_MODEL (falling back to
    GENERATION_MODEL). Always reads current environment/.env values and builds a fresh
    client -- nothing is cached across calls."""
