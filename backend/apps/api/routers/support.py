...
# Caps so a widget left open in a background tab can't grow context (and
# Groq token spend) unbounded.
...
async def support_chat_ws(websocket: WebSocket) -> None:
    """Support chat for the bottom-right widget. Works for logged-out
    visitors (no token needed) and logged-in users alike -- if the
    frontend passes ?token=<session token>, matching queries/answers get
    attributed to that account in support_chat_logs; otherwise they're
    logged with user_id = NULL, same as before.

    Conversation state is in-memory per connection; every query/answer
    pair is separately persisted to support_chat_logs for later review.
    """
    ...
    try:
        while True:
            ...
            # Sync DB write -- run off the event loop so it never stalls
            # streaming for this or any other concurrent widget session.
            # Best-effort: a logging failure shouldn't break the chat itself.
            try:
